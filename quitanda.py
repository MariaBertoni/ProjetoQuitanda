...
def cadastro():
    if verifica_sessao:
        ...
        conexao = conecta_database()
        conexao.execute('INSERT INTO produtos (nome_prod, desc_prod, preco_prod, img_prod, id_prod) VALUES (?, ?, ?, ?)', (nome_prod, desc_prod, preco_prod, filename, id_prod))
...
def editprod():
    ...
    conexao = conecta_database()
    if img_prod:
        produto = conexao.execute('SELECT * FROM produtos')
        filename= produto[0]['img_prod']
        img_prod.save("static/img/produtos/"+filename)
        conexao.execute('UPDATES produtos SET nome_prod = ?, desc_prod = ?, preco_prod = ?, img_prod = ? WHERE id_prod = ?',(nome_prod, desc_prod, preco_prod, filename, id_prod))
        conexao.commit() #Confirma a alteração no BD
        conexao.close()
        return redirect('/adm')
...
def busca():
    ...
    title = "Home"
    render_template("home.html", produtos=produtos, title=title)
